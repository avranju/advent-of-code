...
pub fn parse(input: &str) -> Result<Instruction, String> {
    ...
    if !parser.expression() {
        ...
        return Err(format!(
            "Invalid input syntax at line {}, column {}",
            line_no, col_no
        ));
    }
...
fn parse_instruction<F>(
    input: &str,
    tokens: &Vec<Token<Rule>>,
    start_index: usize,
    make_instr: F,
) -> Result<Instruction, String>
where
    F: Fn((Coord, Coord)) -> Instruction,
{
...
fn parse_coord(input: &str, tokens: &Vec<Token<Rule>>, index: usize) -> Result<Coord, String> {
    ...
    if tokens.len() <= index + 2 {
        return Err(format!(
            "Invalid input - unexpected end of token stream while parsing coord"
        ));
    }
...
fn parse_number(input: &str, tokens: &Vec<Token<Rule>>, index: usize) -> Result<u32, String> {
    ...
    input
        ...
        .ok_or(format!(
            "Invalid input - unexpected end of input when parsing number"
        ))
        .map(|s| {
            s.parse::<u32>()
                .map_err(|_| format!("Could not parse input as number"))
        })
        .and_then(|r| r)
}
